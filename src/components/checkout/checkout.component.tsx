...
const CheckOut = ({ active, isDropdown, goToCheckOut }: CheckOutProps) => {
  ...
  const itemCount: number = useCartSelector(selectCartItemsCount); 
  const cartItems: Item[] = useCartSelector(selectCartItems); 
  const total: number = useCartSelector(selectCartTotal); 
  const wishlist: number[] = useWishlistSelector(selectWishlistItems); 
