...
import { useAppDispatch, useUserSelector, useCartSelector, useCollectionsSelector, useCategoriesSelector, useWishlistSelector, useAppSelector } from '../../hooks';
...
import { Category, type Item } from '../../models';
...
import { selectIsXs, selectMobileView } from '../../redux/app';
...
const HeaderDesktop = () => {
  ...
  const isMobile = useAppSelector(selectMobileView);
  ...
  const categories: Category[] | null = useCategoriesSelector(selectCategoriesList);
  const wishlistItemCount = useWishlistSelector(selectWishlistItemsCount);

  const transitionsDropdown = useTransition(null, {
    from: { transform: "translateY(-100px)", opacity: 0 },
    enter: { transform: "translateY(-30px)", opacity: 1, zIndex: 40 },
    leave: { transform: "translateY(-100px)", opacity: 0 }
  })

  const transitionSubNavbar = useTransition(null, {
